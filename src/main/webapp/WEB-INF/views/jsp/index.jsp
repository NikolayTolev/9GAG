...
<body>
	...
	<div class="model-upload-p" id="simpleModelPost">
		...
		<div class="upload-model">
			<div class="upload-header">
				<span class="closeBtnPost">&times;</span>
				<form method="post" action="upload/post"
					enctype="multipart/form-data">
					File<input type="file" name="file" required><br>
					Description<input type="text" name="description" required><br>
					Tag<input type="text" name="tag" required><br> <select
						style="width: 179px; height: 30px" name="section" required>
						<c:forEach var="section" items="${sections}">
							<option value="${section.id}">${section.name}</option>
						</c:forEach>
					</select> <input type="submit" value="upload file">
				</form>
			</div>
		</div>
	</div>
